  ...
  if(NRF24L01_ReadReg(NRF24L01_07_STATUS) & _BV(NRF24L01_07_RX_DR)) { // data received from tx
    ...
    else if (packet[0] == 0xA5)
    {
      ...
      if ( (sum&0xFF) == packet[14] )
      {
        ...
        ppm[AILERON] = map(roll, 0, 1022, PPM_MIN, PPM_MAX);
        ppm[ELEVATOR] = map(pitch, 0, 1022, PPM_MIN, PPM_MAX);
        ppm[THROTTLE] = map(throttle, 0, 1022, PPM_MIN, PPM_MAX);
        ppm[RUDDER] = map(yaw, 0, 1022, PPM_MIN, PPM_MAX);
      }
      else
      {
        //checksum FAIL
